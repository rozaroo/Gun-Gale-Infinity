...
public class WeaponController : MonoBehaviour
{
    ...
    void Start()
    {
        //Inicializar con munici�n y cartuchos de ejemplo
        for (int i = 0; i < 3; i++)
            municion.Push(balasPorCartucho);//A�ade 16 balas al cartucho
        for (int i = 0; i < 2; i++)
            cartuchos.Enqueue(balasPorCartucho); //A�ade 2 cartuchos en total
    ...
    IEnumerator AutomaticShoot()
    {
        while (shooting)
        {
            ...
            {
                //Insertar sonido de que no hay munici�n
            }
            //Restar una bala
